...
- Persistent-layout checks compare DOM identity and retain the same button count and input value across theme changes and client navigation. One theme pane contains all seven mounted, expanded specimens, alongside the Colors, Shape & Spacing and Text foundation areas; switching Light/Dark changes tokens rather than creating independent demo state.
...
- Develop checks cover separation from the hidden canvas/inspector, copying React usage to the browser clipboard and selected-theme token references. Color checks retain both-theme preset application, override/geometry preservation, invalid input rejection, contrast warnings, CSS/JSON export, JSON import and reload persistence. Foundation checks cover the live color-scale swatch, shape/spacing values and typography Text sample, their dedicated Develop references, exported CSS and reload persistence; color edits remain theme-specific while typography and geometry edits synchronize across themes. Old v3 JSON files without the optional foundation fields still import.
...
Automated validation (2026-09-26): production build, lint, 103 unit tests and static-export Chromium smoke with screenshots passed across all 20 routes, including the Colors, Shape & Spacing and Text areas. Desktop light/dark and 375px captures were inspected. Manual gates below remain outstanding; automated success does not close them.
