...
- Route checks directly open all 20 URLs: `/`, `/develop`, and both `/<id>` and `/develop/<id>` for `colors`, `spacing`, `button`, `input`, `card`, `badge`, `switch`, `checkbox`, and `text`. They check real navigation links with `aria-current="page"`, headings, selected inspector fields, header Design/Develop selection, the absence of breadcrumbs and browser Back/Forward.
- Persistent-layout checks compare DOM identity and retain the same button count and input value across theme changes and client navigation. One theme pane contains six standalone component specimens, one Text specimen integrated with the Text foundation, and the Colors and Shape & Spacing areas; Text is not duplicated on the canvas; switching Light/Dark changes tokens rather than creating independent demo state.
...
- Develop checks cover separation from the hidden canvas/inspector, copying React usage to the browser clipboard and selected-theme token references. Color checks retain both-theme preset application, override/geometry preservation, invalid input rejection, contrast warnings, CSS/JSON export, JSON import and reload persistence. Foundation checks cover the live color-scale swatch, shape/spacing values and H1–H6 plus legacy typography in the single Text area, their dedicated Develop references, exported CSS and reload persistence; color edits remain theme-specific while typography and geometry edits synchronize across themes. Old v3 JSON files without the optional foundation fields still import.
- At a 375px emulated viewport, checks exercise all seven components and both token foundation navigation targets in Design and Develop, page-width and header-control overflow, inspector/preview focus links, header theme selection and selected accessible names. Mobile emulation additionally checks document scrolling, touch-action and that canvas touchmove is not canceled. These checks do not prove native mobile pinch-to-zoom or a real touch-scrolling session. CDP accessibility-tree assertions are not a screen-reader session; viewport emulation is not browser-native zoom.

Automated validation (2026-09-26): production build, lint, 106 unit tests and static-export Chromium smoke with screenshots passed across all 20 routes, including the Colors, Shape & Spacing and Text areas. Desktop light/dark and 375px captures were inspected. Manual gates below remain outstanding; automated success does not close them.
